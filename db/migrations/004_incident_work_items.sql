CREATE TABLE incident_work_items (
    incident_id TEXT PRIMARY KEY REFERENCES incidents (incident_id) ON DELETE CASCADE,
    stage TEXT NOT NULL DEFAULT 'COLLECTION' CHECK (stage = 'COLLECTION'),
...
CREATE INDEX incident_work_items_claim_idx
    ON incident_work_items (state, available_at, lease_expires_at, incident_id);

CREATE FUNCTION enqueue_incident_collection_work()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO incident_work_items (incident_id, available_at)
    VALUES (NEW.incident_id, NEW.created_at)
    ON CONFLICT (incident_id) DO NOTHING;
    RETURN NEW;
END;
$$;

CREATE TRIGGER incidents_enqueue_collection_work
AFTER INSERT ON incidents
FOR EACH ROW
EXECUTE FUNCTION enqueue_incident_collection_work();
